...
function describeError(error: unknown): string {
  if (typeof error === "object" && error !== null && "_tag" in error) {
    const tag = typeof error._tag === "string" ? error._tag : "error";
...
const searchEmailsEffect = (query: string) =>
  Effect.sync(() => {
    if (!finder) {
...
const loadEmailsEffect = Effect.gen(function* () {
  ...
  Effect.catchAll((error) =>
    Effect.sync(() => setStatus(`error loading folder: ${describeError(error)}`)),
  ),
);

const openEmailEffect = (id: string) =>
  Effect.gen(function* () {
    selectedEmailId = id;
    clearMainContent();
...
const switchFolderEffect = (name: string, clearSearch = true) =>
  Effect.gen(function* () {
    currentFolder = name;
    selectedEmailId = null;
    if (clearSearch) {
