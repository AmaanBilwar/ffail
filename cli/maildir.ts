...
const safeReaddirEffect = (dir: string) =>
  Effect.tryPromise({
...
export const listFoldersEffect = (maildirPath: string) =>
  Effect.gen(function* () {
    ...
    const scan = (dir: string, prefix: string): Effect.Effect<void, MailDirError> =>
      Effect.gen(function* () {
        ...
        }).pipe(Effect.catchIf(isMissingPathError, () => Effect.succeed([])));
...
export async function listFolders(
  maildirPath: string,
): Promise<{ name: string; unread: number }[]> {
...
export const listEnvelopesEffect = (maildir: string, limit = 100) =>
  Effect.gen(function* () {
    ...
    const parsedEnvelopes = yield* Effect.forEach(
      ...
      ({ file, isRead }) =>
        ...
        }).pipe(Effect.catchTag("MailDirError", () => Effect.succeed(null))),
      { concurrency: 8 },
    );
...
export const getEmailEffect = (maildir: string, filename: string) =>
  Effect.gen(function* () {
